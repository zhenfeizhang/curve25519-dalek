    ...
    use super::{
        scalar_to_sppark_scalar, scalar_to_sppark_scalar_bytes, sw_point_to_sppark_affine,
        sw_point_to_sppark_affine_bytes, msm_curve25519,
    };
    ...
    fn edwards_msm(points: &[curve25519_dalek::edwards::EdwardsPoint], scalars: &[Scalar])
    ...
    fn affine_bytes_match(
        out: &crate::SpparkAffineBytes,
        expected: &crate::SpparkAffineBytes
    ) -> bool {
        if out.x != expected.x {
            return false;
        }
    ...
    fn cuda_msm_matches_cpu() {
        ...
        assert!(affine_bytes_match(&out, &expected_bytes), "unexpected MSM output");
    }
    ...
    fn cuda_single_scalar_mul() {
        ...
        assert!(affine_bytes_match(&out_3, &expected_bytes_3), "BASEPOINT * 3 should match");
        ...
        assert!(affine_bytes_match(&out_base, &expected_bytes_base), "BASEPOINT * scalar should match");
        ...
        assert!(affine_bytes_match(&out, &expected_bytes), "P * 1 should equal P");
        ...
        assert!(affine_bytes_match(&out2, &expected_bytes2), "P * 2 should match");
        ...
        assert!(affine_bytes_match(&out3, &expected_bytes3), "P * scalar should match");
    }
